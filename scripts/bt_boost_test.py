#!/usr/bin/env python3
"""Test reducing position boost factors to shift multiplier distribution lower.
From feature analysis: lower multipliers (x0.6-0.9) have better WR in both months."""
import sys, time
...
# P1 base: H5+AD-LOOSE with SL=0.4 (best so far!)
P1_BASE = {
    **NOISE_BASE,
    'InpSLBufferATR': '0.4',
    'InpMaxPosMult': '2.0',
...
VARIANTS = [
    ('REF', 'REF-P1(SL0.4 boost full)', {}),

    # Reduce boost factors (default: BoostIn1HOB=2.0, DeepEntryBoost=1.5)
    ('B1', 'BoostIn1HOB=1.0 DeepEntry=0.75', {
        'InpBoostIn1HOB': '1.0', 'InpDeepEntryBoost': '0.75'}),
    ('B2', 'BoostIn1HOB=0.5 DeepEntry=0.5', {
        'InpBoostIn1HOB': '0.5', 'InpDeepEntryBoost': '0.5'}),
    ('B3', 'BoostIn1HOB=0 DeepEntry=0(全平仓)', {
        'InpBoostIn1HOB': '0.0', 'InpDeepEntryBoost': '0.0'}),

    # Reduce max cap
    ('B4', 'MaxPosMult=1.5', {'InpMaxPosMult': '1.5'}),
    ('B5', 'MaxPosMult=1.0', {'InpMaxPosMult': '1.0'}),

    # Combined: half boost + lower cap
    ('B6', 'BoostHalf+MaxPosMult=1.5', {
        'InpBoostIn1HOB': '1.0', 'InpDeepEntryBoost': '0.75', 'InpMaxPosMult': '1.5'}),
]

MONTHS = [('2505', '2025.05.01', '2025.05.31'), ('2605', '2026.05.01', '2026.05.31')]
total = len(VARIANTS) * 2
print(f"\n{'='*70}")
print(f"  Boost factor reduction test: {len(VARIANTS)}v x 2m = {total} BTs")
...
for key, label, ov in VARIANTS:
    merged = dict(P1_BASE)
    merged.update(ov)
    set_files[key] = make_set(f'bst_{key}', merged, base='v11xau-qs3.set')
...
for vkey, vlabel, ov in VARIANTS:
    for mkey, mfrom, mto in MONTHS:
        ...
        kill_mt5()
        r = run_bt_silent(f'bst_{vkey}_{mkey}', set_files[vkey], mfrom, mto)
        results[key] = r
        if r:
...
print(f"\n{'='*110}")
print(f"  BOOST REDUCTION RESULTS")
print(f"  REF: 2505=${ref_25:,.0f}({ref_t25}T) 2605=${ref_26:,.2f}({ref_t26}T)")
print(f"{'='*110}")
print(f"\n{'Variant':<42} {'2505 T':>5} {'PF':>5} {'PnL':>10} | {'2605 T':>5} {'PF':>5} {'PnL':>10} | {'Net':>10}  d2605")
print('-' * 110)
