#!/usr/bin/env python3
"""H系列：基于May诊断的改进 — 毒时段过滤+方向修正+OB时段质量"""
...
H_TESTS = [
    # H1: 过滤毒时段 15-20 + 02 + 12 + 23 (MT5时间)
    ('V11XAU-H1', 204810, 'H1:过滤毒时段', {
        **D5B_BASE,
        'InpNoEntryHours': '2,12,15,17,18,19,20,23',
    }),

    # H2: 只保留优质时段 03,08,11,13,14,16
    ('V11XAU-H2', 204809, 'H2:仅优质时段', {
        **D5B_BASE,
        'InpNoEntryHours': '0,1,2,4,5,6,7,9,10,12,15,17,18,19,20,21,22,23',
    }),

    # H3: 过滤毒时段 + G2早BE
    ('V11XAU-H3', 204808, 'H3:毒时过滤+早BE', {
        **D5B_BASE,
        'InpNoEntryHours': '2,12,15,17,18,19,20,23',
        'InpBreakevenR': '0.25',
    ...
    # H5: 关闭状态过滤(避免方向Flip杀) + 保留毒时过滤
    ('V11XAU-H5', 204806, 'H5:关状态+毒时过滤', {
        **D5B_BASE,
        'InpNoEntryHours': '2,12,15,17,18,19,20,23',
        'InpEnableStateFilter': 'false',
    }),

    # H6: 全部组合: 毒时过滤+早BE+关状态
    ('V11XAU-H6', 204805, 'H6:毒时+早BE+关状态', {
        **D5B_BASE,
        'InpNoEntryHours': '2,12,15,17,18,19,20,23',
        'InpBreakevenR': '0.25',
        'InpBreakevenLockR': '0.08',
...
ExecutionMode=0
ShutdownTerminal=1
Report=hseries_{name}_{today_str}
"""
    ini_file.write_text(ini_content)
    kill_mt5_tester()
    for old in MT5_DATA.glob(f'hseries_{name}*.htm'):
        try: old.unlink()
        except: pass
    ...
    time.sleep(2)
    html_files = sorted(MT5_DATA.glob(f'hseries_{name}*.htm'), key=lambda p: p.stat().st_mtime, reverse=True)
...
REF = {
    'D5B': {'2025-10': (395, 72.2, 51297.69), '2026-05': (317, 43.5, 0.72)},
    'G2':  {'2025-10': (395, 74.9, 47985.08), '2026-05': (317, 45.9, 1.39)},
}

print(f"{'策略':<30} {'2025-10(好月)':>28} {'2026-05(坏月)':>28}")
print("-" * 90)
for ref_name in ['D5B', 'G2']:
    r10 = REF[ref_name]['2025-10']; r05 = REF[ref_name]['2026-05']
    print(f"{ref_name:<30} {r10[0]:>4}t {r10[1]:>5.1f}% ${r10[2]:>10,.0f}  {r05[0]:>4}t {r05[1]:>5.1f}% ${r05[2]:>10,.2f}")

for ver, magic, desc, params in H_TESTS:
    ...
    s10 = f"{r10[0]:>4}t {r10[1]:>5.1f}% ${r10[2]:>10,.0f}" if r10 else "FAILED"
    s05 = f"{r05[0]:>4}t {r05[1]:>5.1f}% ${r05[2]:>10,.2f}" if r05 else "FAILED"
    v10 = f"{(r10[2]-51297.69)/51297.69*100:+.1f}%" if r10 else "-"
    v05 = f"{(r05[2]-0.72)/0.72*100:+.0f}%" if r05 and r05[2] > 0 else "-"
    print(f"  vs D5B:       {v10:>10}          {v05:>10}")

# Cleanup
for ver, _, _, _ in H_TESTS:
