...
ExpertParameters=v11xau-q2-revised-noise.set
Symbol=XAUUSDm
Period=M1
Model=4
Optimization=0
...
print("Q2-Rev+NOISE 720-day...", end='', flush=True)
proc = subprocess.Popen([MT5_TERMINAL, f'/config:{INI_DIR/"backtest.ini"}'],
...
if htm.exists():
    ...
    print(f'\nQ2-Rev+NOISE: {len(trades)}T, WR={wr:.1f}%, PF={pf:.2f}, PnL=${sum(trades):.2f}, Bal=${300+sum(trades):.2f}')
else:
    print("FAILED")
