...
| 2026-06-02 | v1.0 | 初始规范：10 问题记录 + 6 步流程 + 安全边界 | ✅ 已发布 |

| 2026-06-02 | — | P0 修复：macOS 回测 TF 匹配（`bar_period_min`/`bar_tf` 支持） | ✅ 已提交 |

| 2026-06-02 | — | P0 加固：QS 基础版 anchor 安全限制（并发5/重入5/冷却3min） | ✅ 已提交 |

| 2026-06-02 | — | P1 修复：QS 默认开启入场诊断日志 | ✅ 已提交 |
| 2026-06-02 | — | .set 重新生成：QS/QS2/QS-DEFENSIVE 到 portable Presets | ✅ 已生成 |
| 2026-06-02 | — | QS Live 切换为 QS2 防御版 | ⚠️ 需手动重启终端 |
| 2026-06-02 | — | M1 回测验证（QS2 720 天） | ⏳ 待运行 |
| 2026-06-02 | — | 策略一致性自动化检查脚本 | 📋 待开发 |
...
自 2026-06-25 起，研究版策略的最终毕业结论统一使用 `WFYS`。

固定要求：

- 同时给出 `24独立月` 与 `720d连续回测`
- 先给 `WFYS硬门槛`
- 再给 `WFYS 100分评分`
- 最后给 `WFYS分级结论`
