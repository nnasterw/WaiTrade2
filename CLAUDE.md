...
- 所有 SMC/ICT 策略概念统一用中文表示（订单块、流动性 sweep、结构突破等）。
...
- **Live 必须开启 `InpEnableEntryDebug=true`**；出场诊断 (`InpEnableExitDebug`) 仅在调试时临时开启。
- **禁止 Live 热替换策略版本**（不同版本可能共享 OB 缓存导致行为异常）；换版本必须重启 EA。
- 重大发现/事故必须更新策略迭代规范：`research/notes/2026-06-02_strategy_iteration_spec.md`。
...
当前 XAU 实盘部署（Exness-MT5Trial5, 账号 277656700）：

```
temp/mt5_portable_xau_zd_qs/
├── ZD/                              # 振荡腿 terminal
│   ├── terminal64.exe               # 独立副本（多实例要求）
│   ├── v11xau_live_ZD_startup.ini
│   └── MQL5/Presets/v11xau-zd.set   # InpBarTF=3 (M3)
├── QS/                              # 趋势腿 terminal
│   ├── terminal64.exe               # 独立副本
│   ├── v11xau_live_QS2_startup.ini
│   └── MQL5/Presets/v11xau-qs2.set  # InpBarTF=1 (M1)
└── portfolio_manifest.yaml
```

| 腿 | 品种/TF | 策略版本 | Magic | 风险 | 最大手数 |
|----|---------|----------|-------|:---:|:---:|
| ZD | XAUUSDm M3 | V11XAU-ZD | 204558 | 2% | 0.5 |
| QS | XAUUSDm M1 | V11XAU-QS2 | 204899 | 3% | 0.1 |

**多实例要求**：每个 portable 终端必须独立复制 `terminal64.exe`，同一 exe 只能有一个 `/portable` 实例。
...
代数: V11 = 第 11 代 EA 架构
品种: XAU / BTC / CRY
类型: QS(趋势) / ZD(振荡) / FAGE(自适应) / Rxxx(实验编号)
变体: 2 / DEFENSIVE / ULTRA / HOLD-R1 / HIGHPRICE-LOWBAL-...

继承链示例 (v11xau-qs):
  defaults → v11b_xau_r31_m1_tp15 → v11b_xau_r35_m1_tp15_nomonth
  → v11xau_trend → v11xau_r39_trend_lot5 → v11xau-qs
```
...
- 策略迭代规范：`research/notes/2026-06-02_strategy_iteration_spec.md`
...
- **回测 Period ≠ Live BarTF**：回测脚本必须从策略 `bar_period_min`/`bar_tf` 取 Period，禁止硬编码 M5 覆盖 EA 的 InpBarTF。
- **.set 加载失败 → EA 默认参数裸奔**：部署后必须检查 EA 日志确认 InpVersion 匹配，无文件错误码 [2]。
- **不同版本热替换**：共享 OB 缓存 + 不同参数 → 行为不可预测，必须重启。
- **多实例 terminal64.exe**：同一 exe 只能一个 /portable 实例，多腿需各自独立复制。
...
- OB 核心区间用实体，不用影线。
- 负余额解析必须支持负号。
- 小资金($200)参数失控：`max_pos_mult=200` + 深度入场 boost → 仓位瞬间 x2.6。
