...
```bash
# === 编译EA ===
python scripts/mt5_compile.py WaiTrade2/WaiTrade_OB   # 单个EA
...
scripts/
  mt5_common.py          共享模块: 配置加载/策略解析/日志解析/统计计算/报告格式化
                         + write_set_file/read_agent_log/backtest_main 跨平台共享
  yaml_to_set.py         YAML→.set转换 + NON_STRATEGY_KEYS/FLAT_MAP/TRAIL_MAP 定义
  mt5_compile.py         EA编译: 源码同步→metaeditor64编译→Tester同步 (Wine only)
  mt5_cli_backtest.py    macOS Wine 回测 (kill+wineserver -k清理进程树)
  mt5_backtest_win.py    Windows 原生回测 (taskkill+clear_cache冷启动)
  mt5_live_runner.py     EA Live 部署: 生成.chr profile→编译EA→启动Wine MT5终端
tests/
  test_mt5_common.py     54个pytest: 日志解析/统计/策略解析/报告/参数映射/共享函数
  test_mt5_backtest_win.py  18个pytest: INI生成/进程管理/缓存清理/run流程
...
`strategy_versions/` 目录包含 v6.6~v9.9 的策略文档（回测结果、核心配方、入场/出场逻辑、演进路径）。YAML 中对应版本的参数必须与文档对齐。
...
| **Wine孤儿进程** | pkill只杀terminal64/metatester64，但Wine进程树有45+子进程(wine-preloader/wineserver/services.exe)全泄漏。修复：kill_mt5()加`wineserver -k`+回测结束后再次清理 |
| **Windows回测40x慢** | Windows版INI缺少`[Common]`段(无代理注入)→Model 4 Real Ticks直连海外broker=2h。修复: INI加`[Common]`注入SOCKS5代理 |
...
- Windows回测冷启动机制(taskkill+clear_cache)对齐Wine版
- Windows INI加[Common]段注入代理(修复40x慢)
- Wine孤儿进程修复(wineserver -k + 结束时清理)
- 回测脚本共享代码提取(write_set_file/read_agent_log/backtest_main)
- v10 TP2R实验系列 + OBHeightTP量度移动止盈
