...
void ManagePositions(PosTrack &tracks[], int &track_count, const EAState &state)
{
    ...
    for(int i = track_count - 1; i >= 0; i--)
    {
        ...
        if(!(tracks[i].htf_target && InpHTFSkipDTP))
            ...
            CheckBigWinLock(tracks[i], state);
                  ...
                  PosTrack &tracks[], int &track_count)
{
    ...
    int min_bars = UseBTCProfile() ? InpBTCCapLossMinBars : 0;
    if(cap_r >= 0.0) return;  // 0或正数=禁用
    if(!PositionSelectByTicket(track.ticket)) return;
    // 最短持有bars过滤: 避免入场即被cap
    if(min_bars > 0)
    {
        int bars_held = state.bar_count - track.open_bar;
        if(bars_held < min_bars) return;
    }
    MqlDateTime dt;
    TimeCurrent(dt);
    ...
    double current_price = PositionGetDouble(POSITION_PRICE_CURRENT);
    double current_r = PriceToR(current_price, track.entry_price, track.risk_price, track.direction);
    // 最小亏损阈值: 防止spread在入场时触发cap (需要实际亏损 > 0.1R)
    if(current_r > -0.1) return;
...
// BTC Big Win Lock: when R >= threshold, lock partial profit (50% close)
void CheckBigWinLock(PosTrack &track, const EAState &state)
{
    if(!CfgBTCBigWinLock()) return;
    if(track.bigwin_lock_active) return; // already locked
    double lock_r = CfgBTCBigWinLockR();
    if(lock_r >= 999.0) return; // disabled
    if(!PositionSelectByTicket(track.ticket)) return;
    double current_price = PositionGetDouble(POSITION_PRICE_CURRENT);
    double current_r = PriceToR(current_price, track.entry_price, track.risk_price, track.direction);
    if(current_r < lock_r) return; // not yet big win
    // Close 50% of position to lock partial profit
    double source_volume = PositionGetDouble(POSITION_VOLUME);
    double close_volume = NormalizeDouble(source_volume * 0.5, 2);
    if(close_volume < 0.01) return;
    if(ClosePartial(track.ticket, close_volume, "bigwin_lock"))
    {
        track.bigwin_lock_active = true;
        track.bigwin_lock_volume = close_volume;
